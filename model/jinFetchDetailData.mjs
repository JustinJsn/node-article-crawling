...
import { resolve, readFilePromise, dirIsExists, mkdir } from '../utils/fileSystem.mjs';
import { replaceAsync, myAsyncFn } from '../utils/replaceAndUpload.mjs';
import matchAndUpload from './matchAndUpload.mjs';
...
async function fetchDetailData() {
	try {
		...
		for (let i = 0; i < 1; i++) {
			...
			console.log(chalk.cyan(`开始爬取第 ${i + 1} 条，链接：${parseJson[i].detailUrl}`));
			...
			const purchase = new RegExp('<h1 style="text-align: left;">.*?</p>$', 'ig');
			...
			console.log(chalk.green(`第 ${i + 1} 条爬取成功，休息 2s 开始爬取下一条数据...`));
